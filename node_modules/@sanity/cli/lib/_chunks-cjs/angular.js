"use strict";
var Rt = Object.defineProperty, ze = (r3, e) => {
  for (var t in e)
    Rt(r3, t, { get: e[t], enumerable: !0 });
}, Qe = {};
ze(Qe, { parsers: () => Ve });
var Ve = {};
ze(Ve, { __ng_action: () => Cr, __ng_binding: () => Ar, __ng_directive: () => Nr, __ng_interpolation: () => Or });
var G = class {
  constructor(e, t, s, n) {
...
var ar = ["var", "let", "as", "null", "undefined", "true", "false", "if", "else", "this"], le = class {
  tokenize(e) {
    let t = new be(e), s = [], n = t.scanToken();
    for (; n != null; )
      s.push(n), n = t.scanToken();
    return s;
  }
}, O = class {
  constructor(e, t, s, n, i) {
...
var Ce = new O(-1, -1, f.Character, 0, ""), be = class {
  constructor(e) {
  ...
  scanToken() {
    ...
    for (; s <= 32; )
      if (++n >= t) {
        ...
        s = e.charCodeAt(n);
    if (this.peek = s, this.index = n, n >= t)
      return null;
    if (rt(s))
  ...
  scanNumber(e) {
    ...
    for (this.advance(); ; ) {
      if (!K(this.peek))
        if (this.peek === 95) {
          if (!K(this.input.charCodeAt(this.index - 1)) || !K(this.input.charCodeAt(this.index + 1)))
            return this.error("Invalid numeric separator", 0);
          s = !0;
        } else if (this.peek === 46)
          t = !1;
        else if (xr(this.peek)) {
          if (this.advance(), dr(this.peek) && this.advance(), !K(this.peek))
      ...
      if (this.peek == 92) {
        ...
        if (this.advance(), this.peek == 117) {
          ...
          if (/^[0-9a-f]+$/i.test(g))
            ...
            return this.error(`Invalid unicode escape [\\u${g}]`, 0);
          for (let d = 0; d < 5; d++)
            this.advance();
        } else
          h = fr(this.peek), this.advance();
...
function vr(r3) {
  let e = parseInt(r3);
  if (isNaN(e))
    throw new Error("Invalid integer literal when parsing " + r3);
  return e;
}
var gr = [/^\s*$/, /[<>]/, /^[{}]$/, /&(#|[a-z])/i, /^\/\//];
function ot(r3, e) {
  if (e != null && !(Array.isArray(e) && e.length == 2))
    throw new Error(`Expected '${r3}' to be an array, [start, end].`);
  if (e != null) {
    let t = e[0], s = e[1];
    gr.forEach((n) => {
...
var Te = class r2 {
  ...
  parseTemplateBindings(e, t, s, n, i) {
    ...
    return new U(t, s, i, a, 0, this.errors, 0).parseTemplateBindings({ source: e, span: new I(n, n + e.length) });
  }
  parseInterpolation(e, t, s, n, i = F) {
    let { strings: a, expressions: h, offsets: g } = this.splitInterpolation(e, t, n, i);
    if (h.length === 0)
      return null;
    let d = [];
  ...
  splitInterpolation(e, t, s, n = F) {
    let i = [], a = [], h = [], g = s ? Sr(s) : null, d = 0, y = !1, E = !1, { start: S, end: o } = n;
    for (; d < e.length; )
      if (y) {
        ...
        l.trim().length === 0 && this._reportError("Blank expressions are not allowed in interpolated strings", e, `at column ${d} in`, t), a.push({ text: l, start: u, end: x });
        let k = (g?.get(u) ?? u) + S.length;
        h.push(k), d = x, y = !1;
      } else {
        let u = d;
        d = e.indexOf(S, d), d === -1 && (d = e.length);
        let v = e.substring(u, d);
        i.push({ text: v, start: u, end: d }), y = !0;
      }
    if (!y)
      if (E) {
        let u = i[i.length - 1];
        ...
        i.push({ text: e.substring(d), start: d, end: e.length });
    return new _e(i, a, h);
  }
...
var U = class {
  ...
  parseChain() {
    ...
    for (; this.index < this.tokens.length; ) {
      ...
      if (e.push(s), this.consumeOptionalCharacter(59))
        for (this.parseFlags & 1 || this.error("Binding expression cannot contain chained expression"); this.consumeOptionalCharacter(59); )
          ;
  ...
  parseConditional() {
    ...
    if (this.consumeOptionalOperator("?")) {
      ...
      if (this.consumeOptionalCharacter(58))
        n = this.parsePipe();
      else {
  ...
  parseCallChain() {
    ...
    for (; ; )
      if (this.consumeOptionalCharacter(46))
        ...
        this.consumeOptionalCharacter(40) ? t = this.parseCall(t, e, !0) : t = this.consumeOptionalCharacter(91) ? this.parseKeyedReadOrWrite(t, e, !0) : this.parseAccessMember(t, e, !0);
        ...
        t = new te(this.span(e), this.sourceSpan(e), t);
      else
        return t;
  }
  parsePrimary() {
    ...
    if (this.consumeOptionalCharacter(40)) {
      ...
      if (this.next.isKeywordNull())
        return this.advance(), new C(this.span(e), this.sourceSpan(e), null);
      if (this.next.isKeywordUndefined())
        return this.advance(), new C(this.span(e), this.sourceSpan(e), void 0);
      if (this.next.isKeywordTrue())
        return this.advance(), new C(this.span(e), this.sourceSpan(e), !0);
      if (this.next.isKeywordFalse())
        return this.advance(), new C(this.span(e), this.sourceSpan(e), !1);
      ...
      if (this.consumeOptionalCharacter(91)) {
        ...
        if (this.next.isIdentifier())
          return this.parseAccessMember(new L(this.span(e), this.sourceSpan(e)), e, !1);
  ...
  parseAccessMember(e, t, s) {
    ...
    if (s)
      this.consumeOptionalAssignment() ? (this.error("The '?.' operator cannot be used in the assignment"), h = new $(this.span(t), this.sourceSpan(t))) : h = new z(this.span(t), this.sourceSpan(t), a, e, i);
    else if (this.consumeOptionalAssignment()) {
      if (!(this.parseFlags & 1))
        return this.error("Bindings cannot contain assignments"), new $(this.span(t), this.sourceSpan(t));
      let g = this.parseConditional();
      h = new H(this.span(t), this.sourceSpan(t), a, e, i, g);
    } else
      h = new b(this.span(t), this.sourceSpan(t), a, e, i);
    return h;
  }
  parseCall(e, t, s) {
  ...
  parseKeyedReadOrWrite(e, t, s) {
    return this.withContext(pe.Writable, () => {
      ...
      if (n instanceof $ && this.error("Key access cannot be empty"), this.rbracketsExpected--, this.expectCharacter(93), this.consumeOptionalOperator("="))
        ...
        return s ? new j(this.span(t), this.sourceSpan(t), e, n) : new q(this.span(t), this.sourceSpan(t), e, n);
  ...
  getDirectiveBoundTarget() {
    if (this.next === Ce || this.peekKeywordAs() || this.peekKeywordLet())
      return null;
    let e = this.parsePipe(), { start: t, end: s } = e.span, n = this.input.substring(t, s);
  ...
  skip() {
    let e = this.next;
    for (; this.index < this.tokens.length && !e.isCharacter(59) && !e.isOperator("|") && (this.rparensExpected <= 0 || !e.isCharacter(41)) && (this.rbracesExpected <= 0 || !e.isCharacter(125)) && (this.rbracketsExpected <= 0 || !e.isCharacter(93)) && (!(this.context & pe.Writable) || !e.isOperator("=")); )
      this.next.isError() && this.errors.push(new G(this.next.toString(), this.input, this.locationText(), this.location)), this.advance(), e = this.next;
  }
}, Fe = class extends we {
  constructor() {
...
var ht = "angular-estree-parser", De = "NgEstreeParser", Ue = 0, Ae = [ht, Ue];
function pt() {
  return new fe(new le());
}
function Oe(r3, e) {
  let t = pt(), { astInput: s, comments: n } = wr(r3, t), { ast: i, errors: a } = e(s, t);
  return vt(a), { ast: i, comments: n };
}
function ut(r3) {
  return Oe(r3, (e, t) => t.parseBinding(e, ...Ae));
}
function xt(r3) {
  return Oe(r3, (e, t) => t.parseAction(e, !1, ...Ae));
}
function dt(r3) {
  return Oe(r3, (e, t) => {
...
function ft(r3) {
  let e = pt(), { templateBindings: t, errors: s } = e.parseTemplateBindings(De, r3, ht, Ue, Ue);
  return vt(s), t;
}
function vt(r3) {
  if (r3.length !== 0) {
...
function wr(r3, e) {
  ...
  return t === null ? { astInput: r3, comments: [] } : { astInput: r3.slice(0, t), comments: [{ type: "Comment", value: r3.slice(t + 2), sourceSpan: { start: t, end: r3.length } }] };
}
function gt(r3) {
...
function ct({ start: r3, end: e }, t) {
  let s = r3, n = e;
  for (; n !== s && /\s/.test(t[n - 1]); )
    n--;
  for (; s !== n && /\s/.test(t[s]); )
    s++;
  return { start: s, end: n };
}
function yr({ start: r3, end: e }, t) {
  let s = r3, n = e;
...
function Er(r3, e) {
  return e[r3.start - 1] === "(" && e[r3.end] === ")" ? { start: r3.start - 1, end: r3.end + 1 } : r3;
}
function St(r3, e, t) {
  let s = 0, n = { start: r3.start, end: r3.end };
  for (; ; ) {
    let i = yr(n, e), a = Er(i, e);
    if (i.start === a.start && i.end === a.end)
      break;
    n.start = a.start, n.end = a.end, s++;
  }
  return { hasParens: (t ? s - 1 : s) !== 0, outerSpan: ct(t ? { start: n.start + 1, end: n.end - 1 } : n, e), innerSpan: ct(r3, e) };
}
function mt(r3) {
...
var ve = class {
  ...
  switch (s) {
    case "Unary": {
      let { operator: o, expr: u } = r3, v = n(u);
      return a("UnaryExpression", { prefix: !0, argument: v, operator: o }, r3.sourceSpan, { hasParentParens: t });
    }
    case "Binary": {
      let { left: o, operation: u, right: v } = r3, c = n(o), x = n(v);
      return a(u === "&&" || u === "||" || u === "??" ? "LogicalExpression" : "BinaryExpression", { left: c, right: x, operator: u }, { start: E(c), end: S(x) }, { hasParentParens: t });
    }
    case "BindingPipe": {
      let { exp: o, name: u, args: v } = r3, c = n(o), x = e.getCharacterIndex(/\S/, e.getCharacterIndex("|", S(c)) + 1), l = a("Identifier", { name: u }, { start: x, end: x + u.length }), p = v.map(n);
      return a("NGPipeExpression", { left: c, right: l, arguments: p }, { start: E(c), end: S(p.length === 0 ? l : Ne(!1, p, -1)) }, { hasParentParens: t });
    }
    case "Chain": {
      let { expressions: o } = r3;
      return a("NGChainedExpression", { expressions: o.map(n) }, r3.sourceSpan, { hasParentParens: t });
    }
    case "Comment": {
      let { value: o } = r3;
      return a("CommentLine", { value: o }, r3.sourceSpan, { processSpan: !1 });
    }
    case "Conditional": {
      let { condition: o, trueExp: u, falseExp: v } = r3, c = n(o), x = n(u), l = n(v);
      return a("ConditionalExpression", { test: c, consequent: x, alternate: l }, { start: E(c), end: S(l) }, { hasParentParens: t });
    }
    case "EmptyExpr":
      return a("NGEmptyExpression", {}, r3.sourceSpan, { hasParentParens: t });
    case "ImplicitReceiver":
      return a("ThisExpression", {}, r3.sourceSpan, { hasParentParens: t });
    case "KeyedRead":
    case "SafeKeyedRead": {
      let o = s === "SafeKeyedRead", { key: u } = r3, v = Object.prototype.hasOwnProperty.call(r3, "receiver") ? r3.receiver : r3.obj, c = n(u);
      return h(v, c, { computed: !0, optional: o }, { end: r3.sourceSpan.end, hasParentParens: t });
    }
    case "LiteralArray": {
      let { expressions: o } = r3;
      return a("ArrayExpression", { elements: o.map(n) }, r3.sourceSpan, { hasParentParens: t });
    }
    case "LiteralMap": {
      let { keys: o, values: u } = r3, v = u.map((x) => n(x)), c = o.map(({ key: x, quoted: l }, p) => {
        let k = v[p], m = E(k), N = S(k), ue = e.getCharacterIndex(/\S/, p === 0 ? r3.sourceSpan.start + 1 : e.getCharacterIndex(",", S(v[p - 1])) + 1), It = m === ue ? N : e.getCharacterLastIndex(/\S/, e.getCharacterLastIndex(":", m - 1) - 1) + 1, He = { start: ue, end: It }, ge = l ? a("StringLiteral", { value: x }, He) : a("Identifier", { name: x }, He), kt = ge.end < ge.start || ue === m;
        return a("ObjectProperty", { key: ge, value: k, shorthand: kt, computed: !1 }, { start: E(ge), end: N });
      });
      return a("ObjectExpression", { properties: c }, r3.sourceSpan, { hasParentParens: t });
    }
    case "LiteralPrimitive": {
      let { value: o } = r3;
      switch (typeof o) {
        case "boolean":
          return a("BooleanLiteral", { value: o }, r3.sourceSpan, { hasParentParens: t });
        case "number":
          return a("NumericLiteral", { value: o }, r3.sourceSpan, { hasParentParens: t });
        case "object":
          return a("NullLiteral", {}, r3.sourceSpan, { hasParentParens: t });
        case "string":
          return a("StringLiteral", { value: o }, r3.sourceSpan, { hasParentParens: t });
        case "undefined":
          return a("Identifier", { name: "undefined" }, r3.sourceSpan, { hasParentParens: t });
        default:
    ...
    case "Call":
    case "SafeCall": {
      let o = s === "SafeCall", { receiver: u, args: v } = r3, c = v.length === 1 ? [i(v[0])] : v.map(n), x = n(u), l = d(x), p = o || l ? "OptionalCallExpression" : "CallExpression";
      return a(p, { callee: x, arguments: c, optional: p === "OptionalCallExpression" ? o : void 0 }, { start: E(x), end: r3.sourceSpan.end }, { hasParentParens: t });
    }
    case "NonNullAssert": {
      let { expression: o } = r3, u = n(o);
      return a("TSNonNullExpression", { expression: u }, { start: E(u), end: r3.sourceSpan.end }, { hasParentParens: t });
    }
    case "PrefixNot": {
      let { expression: o } = r3, u = n(o);
      return a("UnaryExpression", { prefix: !0, operator: "!", argument: u }, { start: r3.sourceSpan.start, end: S(u) }, { hasParentParens: t });
    }
    case "PropertyRead":
    case "SafePropertyRead": {
      let o = s === "SafePropertyRead", { receiver: u, name: v } = r3, c = e.getCharacterLastIndex(/\S/, r3.sourceSpan.end - 1) + 1, x = a("Identifier", { name: v }, { start: c - v.length, end: c }, g(u) ? { hasParentParens: t } : {});
      return h(u, x, { computed: !1, optional: o }, { hasParentParens: t });
    }
    case "KeyedWrite": {
      let { key: o, value: u } = r3, v = Object.prototype.hasOwnProperty.call(r3, "receiver") ? r3.receiver : r3.obj, c = n(o), x = n(u), l = h(v, c, { computed: !0, optional: !1 }, { end: e.getCharacterIndex("]", S(c)) + 1 });
      return a("AssignmentExpression", { left: l, operator: "=", right: x }, { start: E(l), end: S(x) }, { hasParentParens: t });
    }
    case "PropertyWrite": {
      let { receiver: o, name: u, value: v } = r3, c = n(v), x = e.getCharacterLastIndex(/\S/, e.getCharacterLastIndex("=", E(c) - 1) - 1) + 1, l = a("Identifier", { name: u }, { start: x - u.length, end: x }), p = h(o, l, { computed: !1, optional: !1 });
      return a("AssignmentExpression", { left: p, operator: "=", right: c }, { start: E(p), end: S(c) }, { hasParentParens: t });
    }
    default:
  ...
  function a(o, u, v, { processSpan: c = !0, hasParentParens: x = !1 } = {}) {
    let l = { type: o, ...Ie(v, e, c, x), ...u };
    switch (o) {
      case "NumericLiteral": {
        let p = l;
        p.extra = { ...p.extra, raw: e.text.slice(p.start, p.end), rawValue: p.value };
        break;
      }
      case "StringLiteral": {
        let p = l;
      ...
      case "ObjectProperty": {
        let p = l;
        p.shorthand && (p.extra = { ...p.extra, shorthand: p.shorthand });
        break;
      }
    }
    return l;
  }
  function h(o, u, v, { end: c = S(u), hasParentParens: x = !1 } = {}) {
    if (g(o) || o.sourceSpan.start === u.start)
      return u;
    let l = n(o), p = d(l);
    return a(v.optional || p ? "OptionalMemberExpression" : "MemberExpression", { object: l, property: u, computed: v.computed, ...v.optional ? { optional: !0 } : p ? { optional: !1 } : null }, { start: E(l), end: c }, { hasParentParens: x });
  }
  function g(o) {
...
function Ie(r3, e, t = !1, s = !1) {
  if (!t) {
    let { start: g, end: d } = r3;
    return { start: g, end: d, range: [g, d] };
  }
  let { outerSpan: n, innerSpan: { start: i, end: a }, hasParens: h } = St(r3, e.text, s);
  return { start: i, end: a, range: [i, a], ...h && { extra: { parenthesized: !0, parenStart: n.start, parenEnd: n.end } } };
}
function $t(r3, e) {
  r3.forEach(o);
  let [t] = r3, { key: s } = t, n = e.text.slice(t.sourceSpan.start, t.sourceSpan.end).trim().length === 0 ? r3.slice(1) : r3, i = [], a = null;
  for (let c = 0; c < n.length; c++) {
    let x = n[c];
    if (a && E(a) && S(x) && x.value && x.value.source === a.key.source) {
      let l = d("NGMicrosyntaxKey", { name: x.key.source }, x.key.span), p = (N, ue) => ({ ...N, ...Ie({ start: N.start, end: ue }, e) }), k = (N) => ({ ...p(N, l.end), alias: l }), m = i.pop();
      if (m.type === "NGMicrosyntaxExpression")
        i.push(k(m));
      else if (m.type === "NGMicrosyntaxKeyedExpression") {
        let N = k(m.expression);
        i.push(p({ ...m, expression: N }, N.end));
      } else
  ...
  return d("NGMicrosyntax", { body: i }, i.length === 0 ? r3[0].sourceSpan : { start: i[0].start, end: Ne(!1, i, -1).end });
  function h(c, x) {
    if (E(c)) {
      let { key: l, value: p } = c;
      return p ? x === 0 ? d("NGMicrosyntaxExpression", { expression: g(p.ast), alias: null }, p.sourceSpan) : d("NGMicrosyntaxKeyedExpression", { key: d("NGMicrosyntaxKey", { name: y(l.source) }, l.span), expression: d("NGMicrosyntaxExpression", { expression: g(p.ast), alias: null }, p.sourceSpan) }, { start: l.span.start, end: p.sourceSpan.end }) : d("NGMicrosyntaxKey", { name: y(l.source) }, l.span);
    } else {
      let { key: l, sourceSpan: p } = c;
      if (/^let\s$/.test(e.text.slice(p.start, p.start + 4))) {
        let { value: m } = c;
        return d("NGMicrosyntaxLet", { key: d("NGMicrosyntaxKey", { name: l.source }, l.span), value: m ? d("NGMicrosyntaxKey", { name: m.source }, m.span) : null }, { start: p.start, end: m ? m.span.end : l.span.end });
      } else {
        let m = v(c);
        return d("NGMicrosyntaxAs", { key: d("NGMicrosyntaxKey", { name: m.source }, m.span), alias: d("NGMicrosyntaxKey", { name: l.source }, l.span) }, { start: m.span.start, end: l.span.end });
      }
    }
  }
  function g(c) {
  ...
  function d(c, x, l, p = !0) {
    return { type: c, ...Ie(l, e, p), ...x };
  }
  function y(c) {
  ...
  function u(c) {
    if (e.text[c.start] !== '"' && e.text[c.start] !== "'")
      return;
    let x = e.text[c.start], l = !1;
    for (let p = c.start + 1; p < e.text.length; p++)
      switch (e.text[p]) {
        case x:
          if (!l) {
            c.end = p + 1;
...
function ke(r3) {
  return (e) => {
    let { ast: t, comments: s } = r3(e), n = new ve(e), i = D(t, n);
    return i.comments = s.map((a) => D(a, n)), i;
  };
}
var Ct = ke(ut), At = ke(dt), Ge = ke(xt), Ot = (r3) => $t(ft(r3), new ve(r3));
function We(r3) {
  var s, n, i;
...
function Re(r3) {
  return { astFormat: "estree", parse(e) {
    let t = r3(e);
    return { type: "NGRoot", node: r3 === Ge && t.type !== "NGChainedExpression" ? { ...t, type: "NGChainedExpression", expressions: [t] } : t };
  }, locStart: We, locEnd: Nt };
}
var Cr = Re(Ge), Ar = Re(Ct), Or = Re(At), Nr = Re(Ot), hs = Qe;
exports.default = hs;
exports.parsers = Ve;
